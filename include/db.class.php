...
class DBConnection {
	 ...
	 * @return boolean
	 */
	function DBConnection($db_host='localhost:3306', $db_user='fetomvof_mazola', $db_pass='mazola123', $db_name='fetomvof_mazola') {
	 ...
	 * @return array
	 */
	function fetchMysqlObject($query) {
	...
	function lock_tables($tables) {
		if (is_array($tables)&&count($tables)>0){
			...
			$this->sendQuery('LOCK TABLES '.$msql.'');
			$this->is_locked=true;
		}
	}
